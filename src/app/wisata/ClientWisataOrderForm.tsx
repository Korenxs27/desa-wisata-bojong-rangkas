...
import { 
  ...
  Upload, X, Download, ArrowRight, Copy, Check, MessageSquare, Calendar, Users, Ticket, CheckCircle, AlertCircle 
} from "lucide-react";
import toast, { Toaster } from "react-hot-toast";

interface WisataOrderFormProps {
  productId: number;
  productName: string;
  productPrice: number;
  statusBuka: string;
...
export default function ClientWisataOrderForm({ 
  productId, 
  productName, 
  productPrice, 
  statusBuka 
}: WisataOrderFormProps) {
  ...
  const isOpen = statusBuka.toLowerCase() === "buka";
  const totalPrice = productPrice * quantity;

  useEffect(() => {
    setMounted(true);
    
    // Cek status login & auto-fill data akun
    ...
    if (!token) {
      toast.error("Silakan login terlebih dahulu untuk memesan tiket objek wisata.", {
  ...
  const handleBooking = async (e: React.FormEvent) => {
    ...
    if (!isOpen) {
      toast.error("Maaf, objek wisata ini sedang tutup/tidak beroperasional.");
      return;
    }

    if (!visitDate) {
      toast.error("Silakan pilih tanggal kunjungan terlebih dahulu.");
      return;
    }
    ...
    const loadToast = toast.loading("Memproses pesanan tiket...");
    ...
    try {
      ...
      if (result.success && result.order_id) {
        ...
        toast.success("Pesanan tiket berhasil dibuat, silakan lakukan pembayaran.");
      } else {
        toast.error(`Gagal membuat pesanan: ${result.message || "Kesalahan server."}`);
      }
    } catch (error) {
      toast.dismiss(loadToast);
  ...
  const handleSendToWhatsApp = () => {
    ...
    const finalName = name || invoiceData?.customer_name || "Pengunjung Wisata";
    const finalPhone = phone || invoiceData?.customer_phone || "-";
    const finalBuktiUrl = uploadedBuktiUrl || invoiceData?.bukti_url || "";

    const textMessage = 
`Halo Admin Desa Wisata Bojongrangkas, saya ingin mengkonfirmasi pembayaran tiket kunjungan wisata.

*ID Order:* #${finalOrderId}
*Nama Pemesan:* ${finalName}
*Kategori:* Tiket Objek Wisata
*Destinasi:* ${productName}
    ...
    <>
      ...
      <form onSubmit={handleBooking} className="space-y-4">
        <div className="space-y-1">
          ...
          <input 
            type="text" required placeholder="Nama lengkap pengunjung" value={name} 
        ...
        <div className="space-y-1 p-3 bg-emerald-50/50 rounded-2xl border border-emerald-100">
          <label className="text-[10px] uppercase font-bold text-emerald-700 flex items-center gap-1">
            <Calendar size={12}/> Tanggal Kunjungan Wisata
          </label>
          <input 
        ...
        <div className="space-y-1">
          <label className="text-[10px] uppercase font-bold text-neutral-400 flex items-center gap-1">
            <Users size={12}/> Jumlah Tiket (Orang)
          </label>
          <input 
              ...
              {paymentMethods.map((method) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <input 
                      type="radio" name="payment_method_wisata" 
        ...
        <button 
          type="submit" disabled={isProcessing || !isOpen || paymentMethods.length === 0}
        ...
        >
          <Ticket size={14}/> {isProcessing ? "Memproses Pesanan..." : isOpen ? "Pesan Tiket & Bayar" : "Wisata Sedang Tutup"}
        </button>
      </form>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-md p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 relative my-auto max-h-[90vh] overflow-y-auto">
            ...
            <div className="text-center space-y-1">
              <span className="px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold rounded-full">Selesaikan Pembayaran Tiket</span>
            ...
            <div className="p-6 sm:p-10 overflow-y-auto space-y-6 bg-white text-neutral-800 flex-1">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
                <div>
                  ...
                  <p className="text-xs text-neutral-500">Tiket Masuk Objek Wisata</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs">
                <div className="space-y-1">
                  ...
                  <p className="text-neutral-600">Tanggal Kunjungan: <strong className="text-neutral-800">{invoiceData.tgl_kunjungan}</strong></p>
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 pt-2 border-t">
                <div className="text-[11px] text-neutral-500 max-w-xs space-y-1">
                  ...
                  <p>Tunjukkan invoice ini kepada petugas loket di gerbang masuk Desa Wisata Bojongrangkas pada hari kunjungan Anda.</p>
                </div>
